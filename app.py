...
def predict_disease(patient_data):
    ...
    model_XGC = joblib.load('./xgboost.pkl')
...
def find_hospital(special_m, lati, long):
    ...
    key = '8bKILlbgY7WeSqYdjXdGdHenyGKpbOhKRDCd5kKDkQTWu20qIzzXzbATkUdYPycVMGB3dJZAPmzGpM7tetApMg%3D%3D'
...
with tab1:
    ...
    col160, col161, col162 = st.columns([0.1, 0.1, 0.6])
...
# *********************** ▼ 연습 코딩 Start ▼ ***********************
    ...
    if select_bins == '일자별':
        ...
        group_day = group_day.rename(columns = {'ID': '출동 건수', 'datetime': '일자별'})

        st.bar_chart(data = group_day, x = '일자별', y = '출동 건수', use_container_width = True)

    elif select_bins=='주별':

        group_week = week_list_df.groupby(by='주별', as_index=False)['ID'].count()
        group_week = group_week.rename(columns = {'ID': '출동 건수'})

        st.bar_chart(data = group_week, x = '주별', y = '출동 건수', use_container_width = True)

    else:

        group_month = month_list_df.groupby(by='월별', as_index=False)['ID'].count()
        group_month = group_month.rename(columns = {'ID': '출동 건수'})

        st.bar_chart(data = group_month, x = '월별', y = '출동 건수', use_container_width = True)
    ...
    col233, col234, col235 = st.columns(3)
    
    with col233: # 일간 통계

        group_day_disease = day_list_df.groupby(by='중증질환', as_index=False)['datetime'].value_counts()
        group_day_disease = group_day_disease.rename(columns={"datetime": "일자별"})

        fig = px.pie(group_day_disease, names="중증질환", title="일간 통계", hole=.4)
        fig.update_traces(textposition='inside', textinfo='percent+label')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig, use_container_width=True, width=600, height=400)

    with col234: # 주간 통계

        group_week_disease = week_list_df.groupby(by='중증질환', as_index=False)['주별'].value_counts()
        group_week_disease = group_week_disease.rename(columns={"주별": "주별"})

        fig = px.pie(group_week_disease, names="중증질환", title="주간 통계", hole=.4)
        fig.update_traces(textposition='inside', textinfo='percent+label')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig, use_container_width=True, width=600, height=400)
    ...
    with col235: # 월간 통계
        ...
        group_month_disease = month_list_df.groupby(by='중증질환', as_index=False)['월별'].value_counts()
        group_month_disease = group_month_disease.rename(columns={"월별": "월별"})

        fig = px.pie( group_month_disease, names="중증질환", title="월간 통계", hole=.4)
        fig.update_traces(textposition='inside', textinfo='percent+label')
        fig.update_layout(font=dict(size=16))
        st.plotly_chart(fig, use_container_width=True, width=600, height=400)
    ...
    with col250:
        jin = data.copy()
        jin['연령구분'] = pd.cut(data['나이'], bins=[-1, 1, 10, 20, 30, 40, 50, 60, 70, 80, 90, 150], 
                     labels=['신생아', '10대미만', '10대','20대','30대','40대','50대','60대','70대','80대','90이상'], right=False)
        a = jin.groupby(by = ['연령구분'], as_index = False)['중증질환'].count()
    
        fig = px.bar(a, y = ['중증질환'], x = ['신생아', '10대미만', '10대','20대','30대','40대','50대','60대','70대','80대','90이상'], text_auto = True, title = "연령대별 중증질환자 비율")
        st.plotly_chart(fig)

    with col251:

        age = st.radio("연령대", ('30대', '40대', '50대'), horizontal=True)

        if age == '30대':    
            group_30 = jin[jin['연령구분']=='30대'].groupby('중증질환', as_index=False)['ID'].count()
            group_30 = group_30.rename(columns={'ID':'명'})
            st.line_chart(data=group_30, x='중증질환', y='명', use_container_width=True)

        elif age == '40대':    
            group_40 = jin[jin['연령구분']=='40대'].groupby('중증질환', as_index=False)['ID'].count()
            group_40 = group_40.rename(columns={'ID':'명'})
            st.line_chart(data=group_40, x='중증질환', y='명', use_container_width=True)

        else:
            group_50 = jin[jin['연령구분']=='50대'].groupby('중증질환', as_index=False)['ID'].count()
            group_50 = group_50.rename(columns={'ID':'명'})
            st.line_chart(data=group_50, x='중증질환', y='명', use_container_width=True)
